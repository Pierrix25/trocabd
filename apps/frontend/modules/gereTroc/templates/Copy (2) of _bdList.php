...
<?php foreach ($thePager->getResults() as $obj): ?>
    <?php echo link_to($obj->getAlbum()->getTalbum(), 'gereBD/show?id='.$obj->getAlbum()->getId().'&sortTroc=Collection').link_to_remote(' + ',array('update' => $contexte, 'url' => 'gereTroc/add'.$contexte.'?albumId=bd_'.$obj->getAlbum()->getId().'&titre='.$obj->getAlbum()->getTalbum())) ?>
    <?php echo '<br>' ?>
<?php endforeach; ?>
