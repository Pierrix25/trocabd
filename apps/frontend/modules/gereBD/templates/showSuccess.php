...
<td><?php echo image_tag(UPLOAD_BD_DIR_REL.'/'.$bd->getPhoto1(), 'size=150x200') ?></td>
<td>
Commentaires:
<?php if ($sf_user->isAuthenticated()): ?>
	<div id="error"></div>
	<?php echo form_remote_tag(array(
	'update' => array('success' => 'comment_list', 'failure' => 'error'),
	'url' => 'gereBD/addCommentaire',
	'position' => 'top',
	'script' => true
	)); ?>
	<?php echo input_hidden_tag('note', $note) ?>
...
<?php 	$commentCriteria = new Criteria(); ?> 
<?php	$commentCriteria->addDescendingOrderByColumn(CommentairealbumPeer::CREATED_AT); ?>
<?php foreach ($bd->getCommentairealbums($commentCriteria) as $comm): ?>
	<?php include_partial('commentaires', array('comm' => $comm)) ?>
<?php endforeach; ?>

</td>
<tr><td> </td><td> </td><td rowspan="20"><?php echo image_tag(UPLOAD_BD_DIR_REL.'/'.$bd->getPhoto2(), 'size=150x200') ?></td></tr>
...
if ($bd->getEditeur()) {
	echo link_to($bd->getEditeur()->getNom(), 'gereBD/showEditeur?id='.$bd->getEditeur()->getId().'&sort='.$sf_request->getParameter('sort').'&rechercher='.$sf_request->getParameter('rechercher'));
  	if ($bd->getEditeur()->getUtilisateur()) {
		if (($sf_user->getAttribute('id')==$bd->getEditeur()->getUtilisateur()->getId())and ($sf_user->hasCredential('manager'))) {
...
foreach ($rScenariste as $scenariste) {
			echo link_to($scenariste->getAuteur()->getNom(), 'gereBD/showAuteur?id='.$scenariste->getAuteur()->getId().'&sort='.$sf_request->getParameter('sort').'&rechercher='.$sf_request->getParameter('rechercher'));
	  		if ($scenariste->getAuteur()->getUtilisateur()) {
				if (($sf_user->getAttribute('id')==$scenariste->getAuteur()->getUtilisateur()->getId()) and ($sf_user->hasCredential('manager'))) {
...
foreach ($rColoriste as $coloriste) {
	echo link_to($coloriste->getAuteur()->getNom(), 'gereBD/showAuteur?id='.$coloriste->getAuteur()->getId().'&sort='.$sf_request->getParameter('sort').'&rechercher='.$sf_request->getParameter('rechercher'));
  	if ($coloriste->getAuteur()->getUtilisateur()) {
		if (($sf_user->getAttribute('id')==$coloriste->getAuteur()->getUtilisateur()->getId())  and ($sf_user->hasCredential('manager'))) { 
...
foreach ($rDessinateur as $dessinateur) {
	echo link_to($dessinateur->getAuteur()->getNom(), 'gereBD/showAuteur?id='.$dessinateur->getAuteur()->getId().'&sort='.$sf_request->getParameter('sort').'&rechercher='.$sf_request->getParameter('rechercher'));
  	if ($dessinateur->getAuteur()->getUtilisateur()) {
		if (($sf_user->getAttribute('id')==$dessinateur->getAuteur()->getUtilisateur()->getId()) and ($sf_user->hasCredential('manager'))) {
