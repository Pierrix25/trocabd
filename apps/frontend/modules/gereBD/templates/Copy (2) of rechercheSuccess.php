...
<tr class="listeObjetLigneTableau">
	...
	<td colspan="2" class="listeTroqueurLigneTableau" >
 	<?php foreach ($obj->getPossedes() as $poss): ?>
			<?php if(($sf_user->hasCredential('troqueur')) and ($poss->getUtilisateur()->getId() <> $sf_user->getAttribute('id'))): ?>
		 		<?php echo link_to ($poss->getUtilisateur()->getLogin(), 'gereTroc/create?sortTroc=created_at&login='.$poss->getUtilisateur()->getLogin().'&utilid='.$poss->getUtilisateurId().'&id=possede_'.$poss->getId().'&albumId=bd_'.$poss->getAlbum()->getId().'&titre='.$poss->getAlbum()->getTalbum())  ?>
			<?php else: ?>
