...
abstract class BaseGenre extends BaseObject  implements Persistent {
	...
	public function setNom($v)
	{
		...
		if ($this->nom !== $v || $v === 'null') {
			$this->nom = $v;
			$this->modifiedColumns[] = GenrePeer::NOM;
		}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(GenrePeer::NOM)) $criteria->add(GenrePeer::NOM, $this->nom);
		...
		if ($this->collAlbums === null) {
			if ($this->isNew()) {
			   $this->collAlbums = array();
				...
				$criteria->add(AlbumPeer::GENRE_ID, $this->getId());
